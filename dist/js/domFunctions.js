...
export const updateDisplay = (weatherJson, locationJson) => {
    fadeDisplay()
    clearDisplay()
...
const clearDisplay = () => {
    ...
    const sixDayForecast = document.getElementById("dailyForecast__conditions");
...
const buildScreenReaderWeather = (weatherJson, locationObj) => {
       const location = locationJson.getName();
       const unit = locationJson.getUnit();
...
const setFocusOnSearch = () => {
    document.getElementById("saerchBar__text").focus();
}
...
const translateIconToFontAwesome = (icon) => {
    ...
    switch(firstTwoChars) {
        ...
        case "02":
            if (lastChar === "d") {
                i.classList.add("fas", "fa-acloud-sun");
            } else {
        ...
        case "10":
            if (lastChar === "d") {
                i.classList.add("fas", "fa-acloud-sun-rain");
            } else {
...
const sixDayForecast = (weatherJson) => {
    for( let i=1; i<=6; i++) {
...
const createDailyForecastDivs = (dayWeather) => {
    ...
    const dayIcon = createDailyForecastIcon(dailyWeather.weather[0].icon, dailyWeather.weather[0].description);
